...
export const MODULE_15_DEVOPS_LESSONS = [
  ...
  // ─── LEÇON 3 : Monitoring et alerting ───
  {
    ...
    duration: "20 min",
    description: "Mettre en place Vercel Analytics pour les performances, les logs Vercel pour le tracking d'erreurs, et le monitoring d'uptime pour etre alerte avant tes clients.",
    content: blocks([
      ...
      { id: "3-5", type: "heading", level: 2, text: "Vercel Logs : le tracking d'erreurs" },
      { id: "3-6", type: "text", html: "<p>Vercel Logs capture automatiquement toutes les erreurs de tes fonctions serverless et de tes API routes. Tu vois les <strong>erreurs en temps reel</strong> dans le dashboard Vercel > Deployments > Function Logs. Pour le cote client, utilise un error boundary React et envoie les erreurs vers une API route de logging.</p><pre><code># Pas d'installation necessaire !\n# Vercel Logs est integre automatiquement.\n# Dashboard > ton projet > Logs\n# Tu vois les erreurs serveur en temps reel.</code></pre>" },
      { id: "3-7", type: "steps", steps: [
        { title: "Erreurs serveur (automatique)", description: "Vercel capture automatiquement les erreurs de tes API routes et fonctions serverless. Tu les vois dans le dashboard Logs." },
        { title: "Erreurs client (error boundary)", description: "Cree un error boundary React qui envoie les erreurs client vers une API route /api/log-error. Claude Code peut te generer ca en 2 minutes." },
        { title: "Alertes", description: "Configure des alertes email dans Vercel Settings > Notifications quand une erreur apparait ou quand le taux d'erreur depasse un seuil." },
        { title: "Logs structures", description: "Utilise console.error() avec des objets structures dans tes API routes. Vercel affiche ces logs avec le contexte complet (timestamp, route, duree)." },
      ]},
      ...
      { id: "3-14", type: "callout", variant: "tip", html: "<strong>Stack recommandee :</strong> Vercel Logs (erreurs) + Vercel Analytics (performance) + UptimeRobot (disponibilite). Le tout integre et gratuit. Tu couvres 90% des besoins de monitoring sans outil tiers." },
    ]),
    exercise: "<p><strong>Exercice :</strong> (1) Deploie ton projet sur Vercel et ouvre le dashboard Logs. (2) Cree une erreur volontaire dans une API route (throw new Error('Test monitoring')) et verifie qu'elle apparait dans les logs Vercel. (3) Active Vercel Analytics dans le dashboard. (4) Cree un monitor UptimeRobot pour l'URL de ton app de prod.</p>",
    quiz: [
      { type: "mcq", question: "Que capturent les Vercel Logs automatiquement sans code supplementaire ?", options: JSON.stringify(["Uniquement les console.log", "Les erreurs des API routes et fonctions serverless", "Les requetes SQL lentes", "Le trafic reseau entrant"]), correctAnswer: "Les erreurs des API routes et fonctions serverless", explanation: "Vercel Logs capture automatiquement toutes les erreurs de tes API routes et fonctions serverless. Pour les erreurs cote client, tu crees un error boundary React qui envoie les erreurs vers une API route de logging." },
      { type: "true_false", question: "Tu dois savoir qu'il y a un probleme AVANT que ton client t'appelle.", options: JSON.stringify(["Vrai", "Faux"]), correctAnswer: "Vrai", explanation: "C'est la regle d'or du monitoring. Si ton client te signale un bug que tu n'as pas detecte, tu perds en credibilite. Le monitoring (Vercel Logs + UptimeRobot) te permet d'etre proactif et de corriger avant que ca impacte les utilisateurs." },
      ...
      { type: "true_false", question: "La stack Vercel Logs + Vercel Analytics + UptimeRobot est entierement payante.", options: JSON.stringify(["Vrai", "Faux"]), correctAnswer: "Faux", explanation: "Les trois sont gratuits : Vercel Logs (inclus), Vercel Analytics (inclus), UptimeRobot (50 monitors en plan gratuit). C'est suffisant pour la plupart des projets freelance et petites agences." },
      { type: "mcq", question: "Ton app en production renvoie un ecran blanc pour certains utilisateurs mais tu ne recois aucune alerte. Que te manque-t-il ?", options: JSON.stringify(["Un CDN plus rapide", "Un error boundary React qui envoie les erreurs client vers une API route de logging", "Un serveur plus puissant", "Un meilleur framework CSS"]), correctAnswer: "Un error boundary React qui envoie les erreurs client vers une API route de logging", explanation: "Un ecran blanc est souvent cause par une erreur JavaScript non geree cote client. Les logs Vercel ne capturent que les erreurs serveur. Pour les erreurs client, tu as besoin d'un error boundary React qui envoie les erreurs vers une API route — que tu peux voir dans les logs Vercel." },
    ],
  },
  ...
  // ─── LEÇON 4 : Gérer les incidents en prod ───
  {
    ...
    content: blocks([
      ...
      { id: "4-6", type: "diagram", variant: "timeline", nodes: [
        { id: "n1", label: "Diagnostiquer", description: "Identifier la cause : Vercel Logs, reproduire le bug" },
      ...
      { id: "4-9", type: "steps", steps: [
        { title: "Verifier les Vercel Logs", description: "Premiere chose : ouvre les Vercel Logs (Dashboard > Logs). Tu y trouveras l'erreur exacte, la stack trace, la route, et le timestamp." },
    ...
    exercise: "<p><strong>Exercice :</strong> (1) Simule un incident : introduis un bug volontaire dans ton app de prod (ou staging). (2) Suis la methode DCCL : diagnostique via les Vercel Logs, corrige avec un hotfix, redige le message client. (3) Fais un rollback Vercel vers la version precedente. (4) Redige un post-mortem de 5 lignes : quoi, pourquoi, impact, fix, prevention.</p>",
    quiz: [
